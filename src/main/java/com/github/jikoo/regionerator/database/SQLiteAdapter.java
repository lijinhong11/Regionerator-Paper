            ...
            try {
                ...
                plugin.getLogger()
                        .warning(String.format(
                                "SQLite driver: %s %s", metaData.getDriverName(), metaData.getDriverVersion()));
                plugin.getLogger()
                        .warning(String.format(
                                "SQLite database: %s %s",
    ...
    public void update(@NotNull Collection<ChunkFlagger.FlagData> flags) throws SQLException {
        synchronized (database) {
            try (PreparedStatement upsert = database.prepareStatement(
                            "INSERT INTO chunkdata(chunk_id,time) VALUES (?,?) ON CONFLICT(chunk_id) DO UPDATE SET time=excluded.time WHERE excluded.time>chunkdata.time");
