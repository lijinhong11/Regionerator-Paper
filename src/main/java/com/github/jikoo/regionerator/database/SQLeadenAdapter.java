...
public class SQLeadenAdapter implements DatabaseAdapter {
    ...
    SQLeadenAdapter(@NotNull Regionerator plugin, @NotNull Connection database) throws SQLException {
        ...
        try (Statement st = database.createStatement()) {
            st.executeUpdate(
                    "CREATE TABLE IF NOT EXISTS `chunkdata`(`chunk_id` TEXT NOT NULL UNIQUE, `time` BIGINT NOT NULL)");
        }
    ...
    public void update(@NotNull Collection<ChunkFlagger.FlagData> flags) throws SQLException {
        synchronized (database) {
            try (PreparedStatement boyIWishThisWasAnUpsert = database.prepareStatement(
                            "INSERT OR REPLACE INTO chunkdata(chunk_id,time) VALUES (?, MAX(COALESCE((SELECT time FROM chunkdata WHERE chunk_id=?),0),?))");
                    PreparedStatement deleteForeverBecauseReplaceEqualsDeleteThenInsertFrownyFace =
                            database.prepareStatement("DELETE FROM chunkdata WHERE chunk_id=?")) {
