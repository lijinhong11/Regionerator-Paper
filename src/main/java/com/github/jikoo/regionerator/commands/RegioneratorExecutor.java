...
public class RegioneratorExecutor implements TabExecutor {
    ...
    public boolean onCommand(
            ...
            for (String worldName : plugin.config().enabledWorlds()) {
                ...
                if (activeAt > System.currentTimeMillis()) {
                    ...
                    if (plugin.config().isRememberCycleDelay()
                            && activeAt >= System.currentTimeMillis() - millisBetweenCycles) {
        ...
        if (sender instanceof Player player && args[0].equals("check")) {
            ...
            for (Hook hook : plugin.getProtectionHooks()) {
                player.sendMessage(
                        "Chunk is " + (hook.isChunkProtected(world, chunk.getX(), chunk.getZ()) ? "" : "not ")
                                + "protected by " + hook.getProtectionName());
            }
            ...
            if (!regionInfo.exists()) {
                long visit = plugin.getFlagger()
                        .getChunkFlag(world, chunk.getX(), chunk.getZ())
                        .join()
                        .getLastVisit();
                if (visit == Config.FLAG_DEFAULT) {
                    player.sendMessage("Chunk has not been visited.");
                } else if (!plugin.config().isDeleteFreshChunks(world)
                        && visit == plugin.config().getFlagGenerated(world)) {
                ...
                visit = plugin.getFlagger()
                        .getChunkFlagOnDelete(world, chunk.getX(), chunk.getZ())
                        .join()
                        .getLastVisit();
                if (visit != Config.FLAG_DEFAULT) {
            ...
            player.sendMessage(
                    "Chunk VisitStatus: " + chunkInfo.getVisitStatus().name());
            long visit = plugin.getFlagger()
                    .getChunkFlagOnDelete(world, chunk.getX(), chunk.getZ())
                    .join()
                    .getLastVisit();
    ...
    public @Nullable List<String> onTabComplete(
        ...
        if ("flag".equals(args[0]) || "unflag".equals(args[0])) {
            ...
            if (args.length == 2) {
                return TabCompleter.completeString(
                        args[1],
                        Stream.concat(Stream.of("selection"), plugin.config().enabledWorlds().stream())
                                .toArray(String[]::new));
            }
