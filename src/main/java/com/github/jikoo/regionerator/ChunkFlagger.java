...
public class ChunkFlagger {
    ...
    private void convertOldFlagsFile() {
        ...
        for (String world : oldFlags.getKeys(false)) {
            ...
            for (String chunkPath : worldSection.getKeys(false)) {
                ...
                flagCache
                        .get(this.getFlagDbId(world, chunkX, chunkZ))
        ...
        if (oldFlagsFile.renameTo(new File(oldFlagsFile.getParentFile(), "flags.yml.bak"))) {
            this.plugin
                    .getLogger()
                    .info(
                            "Finished converting flags.yml, renamed to flags.yml.bak. Delete at convenience if all appears well.");
        } else {
            this.plugin
                    .getLogger()
                    .warning(
                            "Finished converting flags.yml but could not rename! Conversion will run again on startup.");
        }
    }
    ...
    private void convertOldPerWorldFlagFiles() {
        ...
        for (File worldFlagsFolder : worldDirectories) {
            ...
            for (File regionFlagsFile : regionFlagsFiles) {
                ...
                for (Map.Entry<String, Object> entry : values.entrySet()) {
                    ...
                    flagCache
                            .get(this.getFlagDbId(worldName, chunkX, chunkZ))
        ...
        if (oldFlagsFolder.renameTo(new File(oldFlagsFolder.getParentFile(), "flags.bak"))) {
            this.plugin
                    .getLogger()
                    .info(
                            "Finished converting flags folder, renamed to flags.bak. Delete at convenience if all appears well.");
        } else {
            this.plugin
                    .getLogger()
                    .warning(
                            "Finished converting flags folder but could not rename! Conversion will run again on startup.");
        }
    }
    ...
    public void flagChunksInRadius(@NotNull String world, int chunkX, int chunkZ) {
        flagChunksInRadius(
                ...
                chunkX,
                chunkZ,
                this.plugin.config().getFlaggingRadius(),
                this.plugin.config().getFlagVisit(world));
    }
    ...
    public void unflagChunk(@NotNull String world, int chunkX, int chunkZ) {
        flagCache
                .computeIfAbsent(
                        getFlagDbId(world, chunkX, chunkZ), key -> new FlagData(key, Config.FLAG_DEFAULT, true))
    ...
    public static class FlagData {
        ...
        public boolean equals(@Nullable Object obj) {
            ...
            return lastVisit.get() == other.lastVisit.get()
                    && dirty.get() == other.dirty.get()
