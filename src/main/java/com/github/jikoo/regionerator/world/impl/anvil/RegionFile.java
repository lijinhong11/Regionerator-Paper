...
public class RegionFile implements AutoCloseable {
    ...
    public RegionFile(
        ...
        this(
                verifyIntent(whoAreYouAndIsThisSafe, regionPath),
                verifyBuffer(regionHeaderBuffer, REGION_HEADER_LENGTH),
                verifyBuffer(chunkHeaderBuffer, CHUNK_HEADER_LENGTH));
    }

    private RegionFile(
            @NotNull Path regionPath, @NotNull ByteBuffer regionHeaderBuffer, @NotNull ByteBuffer chunkHeaderBuffer) {
        ...
        Matcher matcher =
                FILE_NAME_PATTERN.matcher(this.regionPath.getFileName().toString());
        if (!matcher.matches()) {
            throw new IllegalArgumentException(
                    "Provided region file does not match name format " + regionPath.getFileName());
        }
    ...
    public void readHeader() throws IOException, DataFormatException {
        ...
        if (bytesRead != -1 && bytesRead < REGION_HEADER_LENGTH) {
            throw new DataFormatException(String.format(
                    ...
                    regionPath.getFileName(), bytesRead, REGION_HEADER_LENGTH));
        }
    ...
    private void checkLegalChunks(int chunkX, int chunkZ) {
        ...
        if (regionalX != regionalZ && (regionalZ && regionX != 0 || regionalX && regionZ != 0)) {
            ...
            throw new IllegalArgumentException(
                    "Mismatched local and regional coordinates " + chunkX + " and " + chunkZ);
        }
    }
    ...
    private static Path verifyIntent(@Nullable String password, Path path) {
        // This is a silly little stopgap to ensure that people are at least a little aware that reusing a ByteBuffer
        // can
