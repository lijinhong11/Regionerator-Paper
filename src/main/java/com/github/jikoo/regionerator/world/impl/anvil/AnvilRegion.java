...
public class AnvilRegion extends RegionInfo {
    ...
    AnvilRegion(
        ...
        volatileChunkTimes = volatileRegionHeader
                .slice(RegionFile.SECTOR_BYTES, RegionFile.SECTOR_BYTES)
                .asIntBuffer();
        ...
        storedChunkTimes = storedRegionHeader
                .slice(RegionFile.SECTOR_BYTES, RegionFile.SECTOR_BYTES)
                .asIntBuffer();
    ...
    private boolean write(@NotNull String subdirectory) throws IOException {
        ...
        if (!Files.isRegularFile(mcaFilePath)) {
            getPlugin()
                    .debug(
                            DebugLevel.HIGH,
                            () -> String.format("Skipped nonexistent region %s/%s", subdirectory, getIdentifier()));
        ...
        getPlugin()
                .debug(
                        DebugLevel.HIGH,
                        () -> String.format("Deleted region %s/%s with empty header", subdirectory, getIdentifier()));
        return true;
    }
    ...
    public @NotNull ChunkInfo getLocalChunk(int localChunkX, int localChunkZ) {
        Preconditions.checkArgument(
                localChunkX >= 0 && localChunkX < 32 && localChunkZ >= 0 && localChunkZ < 32,
                "Local chunk coords must be within range 0-31! Received values X: %s, Z: %s",
                localChunkX,
                localChunkZ);
    ...
    private class AnvilChunk extends ChunkInfo {
        ...
        public long getLastModified() {
            return TimeUnit.MILLISECONDS.convert(
                    storedChunkTimes.get(RegionFile.packIndex(getLocalChunkX(), getLocalChunkZ())), TimeUnit.SECONDS);
        }
    }
}
