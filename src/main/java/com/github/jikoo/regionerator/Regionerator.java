...
public class Regionerator extends JavaPlugin {
    ...
    public void onEnable() {
        ...
        getScheduler().runNextTick(t -> {
            ...
            debug(
                    DebugLevel.LOW,
                    () -> executor.onCommand(
                            Bukkit.getConsoleSender(), Objects.requireNonNull(command), "regionerator", new String[0]));
        });
    }
    ...
    public void reloadFeatures() {
        ...
        protectionHooks.removeIf(
                hook -> hook.getClass().getPackage().getName().equals("com.github.jikoo.regionerator.hooks"));
        ...
        Set<String> hookNames = Objects.requireNonNull(
                        Objects.requireNonNull(getConfig().getDefaults()).getConfigurationSection("hooks"))
                .getKeys(false);
        ...
        for (String hookName : hookNames) {
            ...
            try {
                ...
                if (!hook.areDependenciesPresent()) {
                    debug(
                            DebugLevel.LOW,
                            () -> String.format("Dependencies not found for %s hook, skipping.", hookName));
                    continue;
                }
                if (hook.isHookUsable()) {
                    ...
                    getLogger()
                            .warning(
                                    "Protection hook for " + hookName + " failed usability check! Deletion is paused.");
                    setPaused(true);
                }
            } catch (NoClassDefFoundError e) {
                debug(DebugLevel.LOW, () -> String.format("Dependencies not found for %s hook, skipping.", hookName));
    ...
    public void attemptDeletionActivation() {
        ...
        for (String worldName : config.enabledWorlds().stream()
                .sorted(new DeletionStartComparator(miscData))
                .toList()) {
