...
public class BatchExpirationLoadingCache<K, V> {
        ...
        new Thread(
                        () -> {
                            ...
                            if (expired.size() < maxBatchSize) {
                                try {
                                    ...
                                    Regionerator.getInstance()
                                            ...
                                            .log(
                                                    Level.SEVERE,
                                                    "Encountered exception while attempting to await larger batch: ",
                                                    e);
                                }
                            }
                            ...
                            while (expiredValues.size() < maxBatchSize && expiredIterator.hasNext()) {
                                ...
                                V value = expirationMap.contains(expiredKey)
                                        ? internal.get(expiredKey)
                                        : internal.remove(expiredKey);
                                if (value != null) {
    ...
    public static class Builder<K, V> {
        ...
        public @NotNull BatchExpirationLoadingCache<K, V> build(
                @NotNull final Function<K, V> load, @NotNull final Consumer<Collection<V>> expirationConsumer) {
